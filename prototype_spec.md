...
|---|---|---|
| Hero (deal-card framing) | Rendered | Banner with deal title in human terms + giver name. The "gift" ribbon is static; occasion-awareness is not promised in production. |
...
occasion_hint: string | null         # OPTIONAL tone-calibration hint, not load-bearing.
                                     # Notes are unreliable occasion signals in production —
                                     # we do not promise occasion-awareness in the output contract.
                                     # hero_line and scene_paragraph rely only on giver name,
                                     # deal category, and merchant context.
```
